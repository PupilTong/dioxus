...
mod tests {
    ...
    #[test]
    fn infers_image_mime_type() {
        assert_eq!(
            infer_image_mime_type(Path::new("logo.PNG")).as_deref(),
            Some("image/png")
        );
        assert_eq!(
            infer_image_mime_type(Path::new("icon.svg")).as_deref(),
            Some("image/svg+xml")
        );
        assert!(infer_image_mime_type(Path::new("asset.bin")).is_none());
    }
