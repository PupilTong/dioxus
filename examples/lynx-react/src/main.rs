...
use dioxus_lynx::lynx_sys::TimerId;
...
impl FlappyEngine {
    ...
    fn schedule_tick(self: &Rc<Self>) {
        ...
        let timer = dioxus_lynx::lynx_sys::set_timeout(move || engine.tick(), FLAP_FRAME_MS);
    ...
    fn tick(self: Rc<Self>) {
        ...
        if y >= 0.0 {
            ...
            self.on_update.call(0.0);
            return;
        }

        self.y.set(y);
        self.velocity.set(velocity);
        self.on_update.call(y);
        self.schedule_tick();
    }
}
...
fn App() -> Element {
    let mut logo = use_signal(Logo::default);
    let mut logo_y = use_signal(|| 0.0);
    let flappy = use_hook(move || {
        Rc::new(FlappyEngine::new(Callback::new(move |y| {
            logo_y.set(y);
        })))
    });
            ...
            view { class: "App",
                view { class: "Banner",
                    view {
                        class: "Logo",
                        style: logo_style,
                        ontap: move |_| logo.set(logo().toggled()),
                        image {
                            src: logo_value.source(),
                            class: logo_value.class_name(),
                        }
                    }
                    text { class: "Title", "Dioxus" }
                    text { class: "Subtitle", "on Lynx" }
                }
                view { class: "Content",
                    image { src: ARROW, class: "Arrow" }
                    text { class: "Description", "Tap the logo and have fun!" }
                    text { class: "Hint",
                        "Edit"
                        text { style: HINT_EMPHASIS_STYLE, " src/main.rs " }
                        "to see updates!"
                    }
                }
